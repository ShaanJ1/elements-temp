<div class="relative">
	<!-- Background image -->
	<div class="absolute inset-0">
		<img
			class="h-full w-full object-cover"
			src="/images/images/hero/hero-image.webp"
			alt="Custom millwork workshop"
            fetchpriority="high"
		/>
		<div class="absolute inset-0" style="background-color: rgba(0, 0, 0, 0.45);"></div>
	</div>

	<!-- Content -->
	<div class="relative">
		<div class="mx-auto max-w-7xl px-4 py-24 sm:px-6 sm:py-32 lg:px-8">
			<h1 class="text-4xl font-extrabold tracking-tight text-text-inverse sm:text-5xl md:text-6xl">
				<span class="block text-white">Elements Millwork &</span>
				<span class="block text-white">Flooring Inc.</span>
			</h1>
			<p class="mt-6 max-w-lg text-xl text-text-muted sm:max-w-3xl">
				Lorem ipsum dolor sit amet consectetur adipisicing elit. Quasi aliquam hic expedita amet maxime, aut veritatis, nesciunt voluptatem in at.
			</p>
			<div class="mt-10 flex gap-4">
				<a href="/services" class="btn-primary"> Our Services </a>
				<a
					href="/gallery"
					class="inline-flex items-center rounded-md border border-text-inverse px-6 py-3 text-base font-medium text-text-inverse hover:bg-text-inverse hover:text-primary"
				>
					View Gallery
				</a>
			</div>
		</div>
	</div>
</div>
